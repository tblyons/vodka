...
struct array {
  ...
  constexpr void fill(const_reference val) { for (auto& elem : m_elems) { elem = val; } }
  ...
  // element access:
  [[nodiscard]] constexpr auto operator[](size_type n)       noexcept ->       reference { assert(n < N); return m_elems[n]; }
  [[nodiscard]] constexpr auto operator[](size_type n) const noexcept -> const_reference { assert(n < N); return m_elems[n]; }
  [[nodiscard]] constexpr auto         at(size_type n)                ->       reference { if (n >= N) { throw std::out_of_range("array::at"); } return m_elems[n]; }
  [[nodiscard]] constexpr auto         at(size_type n) const          -> const_reference { if (n >= N) { throw std::out_of_range("array::at"); } return m_elems[n]; }
